...
export interface DashboardReport {
  ...
  hourly: {
    buckets: Array<{ hour: number; label: string; total: number; count: number }>;
    peak: number;
    total: number;
  };
  byCategory: { rows: Array<{ label: string; amount: number }>; peak: number; total: number };
  paymentBreakdown: Array<{ method: PaymentMethod; label: string; amount: number; share: number }>;
...
export interface RangeReport {
  ...
  average: number;
  topSellers: Array<{ name: string; units: number; revenue: number }>;
  slowMovers: Array<{ name: string; qty: number; value: number }>;
  staffPerformance: Array<{ name: string; count: number; total: number; avg: number }>;
}
